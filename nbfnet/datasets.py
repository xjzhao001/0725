...
class IndRelLinkPredDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        file_path_write1 = "/home/ubuntu/下载/NBFNet-PyG-master/data/entity2id.txt"
        file_path_write2 = "/home/ubuntu/下载/NBFNet-PyG-master/data/relation2id.txt"
        f_write1 = open(file_path_write1, 'w')
        f_write2 = open(file_path_write2, 'w')
        for key in inv_train_entity_vocab.keys():
            ent = str(inv_train_entity_vocab[key])
            id = str(key)
            f_write1.write(ent)
            f_write1.write("\t")
            f_write1.write(id)
            f_write1.write("\n")
        for key in inv_relation_vocab.keys():
            relation = str(inv_relation_vocab[key])
            id = str(key)
            f_write2.write(relation)
            f_write2.write("\t")
            f_write2.write(id)
            f_write2.write("\n")
        f_write1.close()
